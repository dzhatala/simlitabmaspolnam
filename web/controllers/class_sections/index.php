    ...
    foreach($rows_sql as $row_key => $row_sql){
        for($i = 0; $i < count($table_columns); $i++){
			...
			if($table_columns[$i] == 'BATCH_ID'){
			    $findexternal_sql = "SELECT BATCH_ID, CONCAT(CONCAT(MAJORS.NAME,'-') , BATCHES.NAME) AS LOOKUP FROM BATCHES, MAJORS WHERE BATCHES.MAJOR_ID=MAJORS.MAJOR_ID AND BATCH_ID=?";
			    $findexternal_row = $app['db']->fetchAssoc($findexternal_sql, array($row_sql[$table_columns[$i]]));
			    $rows[$row_key][$table_columns[$i]] = $findexternal_row['LOOKUP'];
			}
			else if($table_columns[$i] == 'ACADEMIC_CALENDAR_ID'){
			    $findexternal_sql = "SELECT ACADEMIC_CALENDAR_ID, CONCAT(CONCAT(YEAR_START,'-') , YEAR_END) AS LOOKUP FROM ACADEMIC_CALENDARS WHERE ACADEMIC_CALENDAR_ID=?";
			    $findexternal_row = $app['db']->fetchAssoc($findexternal_sql, array($row_sql[$table_columns[$i]]));
			    $rows[$row_key][$table_columns[$i]] = $findexternal_row['LOOKUP'];
			}
			else{
...
$app->match('/class_sections/create', function () use ($app) {
	...
	$options = array();
	$findexternal_sql ="SELECT BATCH_ID, CONCAT(CONCAT(MAJORS.NAME,'-') , BATCHES.NAME) AS LOOKUP FROM BATCHES, MAJORS WHERE BATCHES.MAJOR_ID=MAJORS.MAJOR_ID ";
	...
	$options = array();
	$findexternal_sql ="SELECT ACADEMIC_CALENDAR_ID, CONCAT(CONCAT(YEAR_START,'-') , YEAR_END) AS LOOKUP FROM ACADEMIC_CALENDARS  ";
	$findexternal_rows = $app['db']->fetchAll($findexternal_sql, array());
	foreach($findexternal_rows as $findexternal_row){
	    $options[$findexternal_row['ACADEMIC_CALENDAR_ID']] = $findexternal_row['LOOKUP'];
	}
	if(count($options) > 0){
